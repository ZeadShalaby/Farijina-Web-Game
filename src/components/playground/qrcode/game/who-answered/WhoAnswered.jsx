...
function WhoAnswered() {
  ...
  const [count, setCount] = useState(() => {
    if (typeof window !== "undefined") {
      const savedCount = localStorage.getItem("qrcode-game-count");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("qrcode-game-count", count.toString());
    }
  }, [count]);
  ...
  const handleChooseWinner = (e) => {
    ...
    if (isFinished) {
      router.push("/qrcode-playground/result");
    } else {
      router.push("/qrcode-playground");
    }
  };
